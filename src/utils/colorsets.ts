const RAW_COLOR_SETS: { name: string, type: 'div' | 'seq' | 'qual', colors: string[] }[] = [
{
  'name': 'BrBG',
  'type': 'div',
  'colors': [
...
const COLOR_SETS = RAW_COLOR_SETS.map(cs => ({
  name: cs.name,
  type: cs.type,
  colors: cs.colors.map((group) => {
    return group.split(",").map(rgb => [
